...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        self.density_delta = torch.nn.Parameter(torch.zeros(self.density.grid.shape), requires_grad=True)
        self.k0_delta = torch.nn.Parameter(torch.zeros(self.k0.grid.shape), requires_grad=True)
    ...
    def set_deltas(self, deltas):
        self.density_1 = torch.nn.Parameter()
        self.k0_1 = torch.nn.Parameter()
        self.density_1.data = deltas[0]
        self.k0_1.data = deltas[1]
    ...
    @torch.no_grad()
    def scale_volume_grid(self, num_voxels, deltas=None):
        
        self.density_orig = copy.deepcopy(self.density)
        self.k0_orig = copy.deepcopy(self.k0)
        ...
        if deltas is not None:
            self.density_delta.data = deltas[0]
            self.k0_delta.data = deltas[1]
        else:
            self.density_delta.data = torch.zeros(self.density.grid.shape)
            self.k0_delta.data = torch.zeros(self.k0.grid.shape)
    ...
    def update_occupancy_cache(self):
        ...
        t_d = self.density.grid + torch.sum(torch.stack(tuple(self.density_ts)), dim=0)
        d = grid.DenseGrid(1, self.world_size, self.density.xyz_min, self.density.xyz_max, t_d)
        cache_grid_density = d(cache_grid_xyz)[None,None]
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        t_d = self.density.grid + self.density_delta
        t_k = self.k0.grid + self.k0_delta
        
        d = grid.DenseGrid(1, self.world_size, self.density.xyz_min, self.density.xyz_max, t_d)
        k = grid.DenseGrid(12, self.world_size, self.density.xyz_min, self.density.xyz_max, t_k)
