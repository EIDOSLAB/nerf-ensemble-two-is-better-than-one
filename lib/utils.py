...
def load_model_and_pruning_mask(model_class, ckpt_path, coarse=False, multi_nerf=False):
    ckpt = torch.load(ckpt_path)
    pruning_mask = None
    if multi_nerf:
    ...
    if 'pruning_mask' in ckpt.keys():
        pruning_mask = ckpt['pruning_mask']
    model = model_class(**ckpt['model_kwargs'])
    model.load_state_dict(ckpt['model_state_dict'])
    return model, pruning_mask


def load_model(model_class, ckpt_path, coarse=True, multi_nerf=False):
    ckpt = torch.load(ckpt_path)
    if multi_nerf:
        for i in range(len(ckpt_path) - 1, 0, -1):
            if ckpt_path[i] == '/':
